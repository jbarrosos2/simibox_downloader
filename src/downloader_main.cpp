...
static void sd_diag_dump_gpio_state(gpio_num_t pin, const char* name) {
    ...
    ESP_LOGI(TAG_SD, "  GPIO%-2d [%-8s]: level=%d | mode=%s | in_reg=%d",
             pin, name, level,
...
static void sd_diag_dump_all_gpio_states(const char* context) {
    ...
    ESP_LOGI(TAG_SD, "  Pin  Name        Level  Mode    Input");
    ESP_LOGI(TAG_SD, "  ───  ──────────  ─────  ──────  ─────");
...
static bool sd_diag_test_gpio_connectivity(void) {
    ...
    // Test CS pin - should be controllable
    ...
    if (cs_high == 1 && cs_low == 0) {
        ...
        ESP_LOGE(TAG_SD, "    ✗ CS pin NOT toggling! (HIGH=%d, LOW=%d) - Check for short circuit!", cs_high, cs_low);
        all_ok = false;
    }
    ...
    vTaskDelay(pdMS_TO_TICKS(1));
    int rfid_level = gpio_get_level((gpio_num_t)RFID_SS_PIN);
    if (rfid_level == 1) {
        ESP_LOGI(TAG_SD, "    ✓ RFID_SS is HIGH (RFID deselected, won't interfere)");
    } else {
        ESP_LOGE(TAG_SD, "    ✗ RFID_SS stuck LOW - RFID may interfere with SD SPI!");
        all_ok = false;
    }
...
void simibox_download::force_clean_sd_bus() {
    ...
    sd_diag_dump_system_state("System State Before Cleanup");
    sd_diag_dump_all_gpio_states("GPIO States Before Cleanup");
