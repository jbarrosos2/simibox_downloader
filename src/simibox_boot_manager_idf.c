 ...
 * 
 * Uses shared NVS protocol (simibox_nvs_protocol.h) for communication
 * with the musicbox firmware.
 */
...
esp_err_t boot_mgr_init(void) {
    esp_err_t ret = nvs_open(SIMIBOX_NVS_NAMESPACE, NVS_READWRITE, &g_nvs_handle);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to open NVS namespace '%s': %s", 
                 SIMIBOX_NVS_NAMESPACE, esp_err_to_name(ret));
        return ret;
    }
    
    boot_mgr_load_state(&g_current_state);
    
    ESP_LOGI(TAG, "Boot manager initialized (namespace: %s)", SIMIBOX_NVS_NAMESPACE);
    return ESP_OK;
}
...
esp_err_t boot_mgr_save_state(const boot_state_t* state) {
    ...
    // Use shared NVS keys from protocol header
    ret |= nvs_set_u8(g_nvs_handle, NVS_KEY_REASON, (uint8_t)state->reason);
    ret |= nvs_set_str(g_nvs_handle, NVS_KEY_FOLDER, state->folder);
    ret |= nvs_set_u8(g_nvs_handle, NVS_KEY_RETRY, state->retry_count);
    ret |= nvs_set_u8(g_nvs_handle, NVS_KEY_ERROR, (uint8_t)state->last_error);
    ret |= nvs_set_u8(g_nvs_handle, NVS_KEY_VOLUME, state->volume);
    ret |= nvs_set_u8(g_nvs_handle, NVS_KEY_MAX_VOL, state->max_volume);
    ret |= nvs_set_str(g_nvs_handle, NVS_KEY_SSID, state->last_ssid);
    ret |= nvs_set_u32(g_nvs_handle, NVS_KEY_SIZE, state->download_size);
    ret |= nvs_set_u8(g_nvs_handle, NVS_KEY_PROGRESS, state->download_progress);
...
esp_err_t boot_mgr_load_state(boot_state_t* state) {
    ...
    // Load using shared NVS keys
    if (nvs_get_u8(g_nvs_handle, NVS_KEY_REASON, &temp) == ESP_OK) {
        state->reason = (boot_reason_t)temp;
    }
    
    size_t str_len = sizeof(state->folder);
    nvs_get_str(g_nvs_handle, NVS_KEY_FOLDER, state->folder, &str_len);
    
    nvs_get_u8(g_nvs_handle, NVS_KEY_RETRY, &state->retry_count);
    
    if (nvs_get_u8(g_nvs_handle, NVS_KEY_ERROR, &temp) == ESP_OK) {
        state->last_error = (download_error_t)temp;
    }
    
    nvs_get_u8(g_nvs_handle, NVS_KEY_VOLUME, &state->volume);
    nvs_get_u8(g_nvs_handle, NVS_KEY_MAX_VOL, &state->max_volume);
    
    str_len = sizeof(state->last_ssid);
    nvs_get_str(g_nvs_handle, NVS_KEY_SSID, state->last_ssid, &str_len);
    
    nvs_get_u32(g_nvs_handle, NVS_KEY_SIZE, &state->download_size);
    nvs_get_u8(g_nvs_handle, NVS_KEY_PROGRESS, &state->download_progress);
...
esp_err_t boot_mgr_clear_state(void) {
    ...
    ESP_LOGI(TAG, "Clearing boot state");
...
esp_err_t boot_mgr_request_download(const char* folder, uint32_t expected_size) {
    ...
    ESP_LOGI(TAG, "Download requested: folder='%s', expected_size=%lu", 
             folder, (unsigned long)expected_size);
...
esp_err_t boot_mgr_update_progress(uint8_t percent) {
    ...
    esp_err_t ret = nvs_set_u8(g_nvs_handle, NVS_KEY_PROGRESS, percent);
    if (ret == ESP_OK) {
        ret = nvs_commit(g_nvs_handle);
    }
...
esp_err_t boot_mgr_report_success(void) {
    ...
    ESP_LOGI(TAG, "✓ Download SUCCESS reported for folder '%s'", g_current_state.folder);
...
esp_err_t boot_mgr_report_failure(download_error_t error) {
    ...
    ESP_LOGE(TAG, "✗ Download FAILED: %s (retry %d/%d)",
            boot_mgr_get_error_string(error), 
            g_current_state.retry_count,
...
bool boot_mgr_needs_download(void) {
    bool needs = (g_current_state.reason == BOOT_REASON_DOWNLOAD_REQUEST) && 
                 (strlen(g_current_state.folder) > 0);
    
    ESP_LOGI(TAG, "needs_download() = %s (reason=%d, folder='%s')",
             needs ? "YES" : "NO", 
             g_current_state.reason, 
             g_current_state.folder);
...
esp_err_t boot_mgr_set_volume(uint8_t vol) {
    ...
    esp_err_t ret = nvs_set_u8(g_nvs_handle, NVS_KEY_VOLUME, vol);
    if (ret == ESP_OK) {
        ret = nvs_commit(g_nvs_handle);
    }
